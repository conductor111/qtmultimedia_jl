...
bool QGstAppSrc::setup(GstElement* appsrc)
{
    ...
    if (m_useCustomProcessing)
    {
        ...
        bool validCustomProcessing = false;
        GstCaps* caps = ::gst_app_src_get_caps(GST_APP_SRC(appsrc));
        for (gint i = 0; i < ::gst_caps_get_size(caps); i++)
        {
            GstStructure* capsStr = ::gst_caps_get_structure(caps, i);
            if (::strcmp(::gst_structure_get_name(capsStr), "video/x-raw") == 0)
            {
                gint value_numerator;
                gint value_denominator;
                gboolean res = gst_structure_get_fraction(capsStr, "framerate", &value_numerator, &value_denominator);
                if (value_numerator != 0 && value_denominator != 0)
                {
                    m_frameDuration = gst_util_uint64_scale_int(value_denominator, GST_SECOND, value_numerator);
                    validCustomProcessing = true;
                }
                // gchar* c = gst_structure_to_string(capsStr);

                break;
            }
            m_useCustomProcessing = validCustomProcessing;
        }
        ::gst_caps_unref(caps);
    }
...
void QGstAppSrc::on_need_data(GstAppSrc *element, guint arg0, gpointer userdata)
{
    ...
    if (self) {
        ...
        self->dataRequestSize()= self->m_useCustomProcessing ? -1 : arg0;
