...
#if QT_CONFIG(gstreamer_app)
// jl
void QGstreamerPlayerSession::configureAppSrcObjects(QNetworkRequest request, QGstAppSrc *appSrc, GstElement *gstappsrc, QIODevice** appSrcStream)
{
    ...
    for (int i = 0; i < listParams.size(); ++i)
    {
        ...
        if (name == "caps")
        {
            GstCaps *caps = ::gst_caps_from_string(value.toStdString().data());
            if (caps)
            {
                if (gstappsrc)
                {
                ...
                if (appSrc)
                {
                    appSrc->useCustomProcessing() = true;
                }
            }
        }
        else if (name == "send_eos_on_stream_empty_data")
        {
            if (appSrc)
            {
                QString v = value.toLower();
                appSrc->sendEosOnStreamEmptyData() = (v == cTrue || v == cFalse) ? v == cTrue : true;
            }
        }
        else if (name == "max_bytes")
        {
            ...
            {
                bool ok;
                guint64 _val = value.toLongLong(&ok);
                if (ok)
                {
                    ::gst_app_src_set_max_bytes(GST_APP_SRC(gstappsrc), _val);
                }
            }
        }
        else if (name == "appSrcStream")
        {
            if (appSrcStream)
            {
                bool ok;
                *appSrcStream = (QIODevice*)value.toULongLong(&ok);
                if (!ok)
                {
        ...
        else
        {
            if (gstappsrc)
            {
                ...
                if (v == cTrue || v == cFalse)
                {
                    ::g_object_set(gstappsrc, name.toStdString().data(), v == cTrue, NULL);
                }
            }
        }
    }
...
void QGstreamerPlayerSession::configureAppSrcElement(GObject* object, GObject *orig, GParamSpec *pspec, QGstreamerPlayerSession* self)
{
    ...
    // jl
    QVariant userData = self->m_request.attribute(QNetworkRequest::User);
    if (!userData.isNull() && userData.isValid())
    {
...
// jl
GstElement* QGstreamerPlayerSession::buildElement()
{
#if QT_CONFIG(gstreamer_app)
    if (!m_appSrcRecord || !m_appSrcRecord->stream())
    {
        return nullptr;
    }

    QVariant userData = m_request.attribute(QNetworkRequest::User);
    if (userData.isNull() || !userData.isValid())
    {
        return nullptr;
    }
    ...
    if (!m_appSrcRecord->setup(appsrc))
    {
        qWarning() << "Could not setup appsrc (record) element";
    }
...
void QGstreamerPlayerSession::loadFromStream(const QNetworkRequest &request, QIODevice *appSrcStream)
{
    ...
    if (!userData.isNull() && userData.isValid())
    {
        QIODevice* tAppSrcStream = nullptr;
        configureAppSrcObjects(request, nullptr, nullptr, &tAppSrcStream);
        if (tAppSrcStream)
        {
            if (!m_appSrcRecord)
            {
                m_appSrcRecord = new QGstAppSrc(nullptr);
            }
            m_appSrcRecord->setStream(tAppSrcStream);
        }
    }
    //////////////////////////////////////////////////////////////////////////
