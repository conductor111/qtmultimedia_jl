...
QMediaService* QGstreamerPlayerServicePlugin::create(const QString &key)
{
    ...
    if (key == QLatin1String(Q_MEDIASERVICE_MEDIAPLAYER))
    {
        ...
        if (!QGstreamerPlayerService::isPlayerCaptureSessionAvailable(existLibrary, existCreator)) {
            QString baseString = "Error: cannot create player service, ";
            if (!existLibrary)
            {
                qWarning((baseString + "the 'mediacapture' plugin is missing.").toStdString().data());
            }
            else if (existLibrary && !existCreator)
            {
                qWarning((baseString + "the 'mediacapture' plugin doesn't contain 'createQGstreamerCaptureSessionWrapper' function.").toStdString().data());
            }
            else
            {
                qWarning((baseString + "unknown error.").toStdString().data());
            }
            return Q_NULLPTR;
        }
        //////////////////////////////////////////////////////////////////////////
